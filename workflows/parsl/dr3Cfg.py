# dr3Cfg.py - Parsl workflow configuration for running DESC DR3 DDF processing
import dataclasses
import importlib
import os
import sys
...
## Note that --slurmd-debug may take on values of: {quiet,fatal,error,info,verbose}
...
#        nodes_per_block=20,       ## nodes per batch job
        nodes_per_block=25,       ## nodes per batch job
        exclusive=True,
...
cori_knl_2 = HighThroughputExecutor(
    ...
    provider=SlurmProvider(
        ...
        parallelism=0,            ## >0 causes multiple batch jobs, even for simple WFs
        scheduler_options="""#SBATCH --constraint=knl\n#SBATCH --qos=premium""",  ## cori queue
        launcher=SrunLauncher(overrides='-K0 -k --slurmd-debug=error'),
        cmd_timeout=300,          ## timeout (sec) for slurm commands (NERSC can be slow)
        walltime="9:00:00",
        worker_init=worker_init
    ),
...
cori_shifter_debug_config = WorkflowConfig(
    ...
    #repo_dir = "/global/cscratch1/sd/descdm/DC2/DR2/repo",   # DR2 WFD Butler repo
    repo_dir = "/global/cscratch1/sd/descdm/DC2/Run3.1i/DR3/repo", # DR3 DDF Butler repo
...
#    visit_max = 262622,
    ...
    ## First 10 visits in the DR3 repo
    #visit_min = 0,
    #visit_max = 181111,
...
#    root_softs="/global/cscratch1/sd/descdm/ParslRun/",
    root_softs="/global/cscratch1/sd/descdm/ParslDR3/",      #***** FOR DEVELOPMENT *****
