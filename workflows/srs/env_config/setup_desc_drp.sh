...
pip install -c ./dm-constraints-py3-4.5.12.txt --root $1 pyarrow==$DESC_pyarrow_VER
pip install -c ./dm-constraints-py3-4.5.12.txt --root $1 GCR==$DESC_GCR_VER
pip install -c ./dm-constraints-py3-4.5.12.txt --root $1 https://github.com/LSSTDESC/gcr-catalogs/archive/$DESC_GCRCatalogs_VER.tar.gz
...
python setup.py install --prefix=$1/$CONDA_PREFIX
cd ..

export PYTHONPATH=$PYTHONPATH:$1/$CONDA_PREFIX/lib/python3.7/site-packages 
...
echo
echo "When setting up this DMstack env, append "$1"/"$CONDA_PREFIX"/lib/python3.7/site-packages to the PYTHONPATH"
